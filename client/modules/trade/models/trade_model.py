...
class TradeModel:
    ...
    def get_stock_sector(self, symbol):
        """שליפת הסקטור דרך ה-API Client"""
        # משתמשים בפונקציה הקיימת ב-APIClient שמביאה info
        info = self.api.get_live_quote(symbol) # או פונקציה ייעודית אם יש ב-api_client
        if info:
            return info.get('sector', 'Technology')
        return "Technology"

    def send_trade_request(self, mode, data):
        """שליחת בקשת קנייה/מכירה דרך הצינור המרכזי"""
        # mode = 'buy' או 'sell'
        return self.api.post_trade(mode, data)

    def get_saved_cards(self, user_id):
        """שליפה ועיבוד כרטיסים"""
        response = self.api.get_saved_cards(user_id)
        
        if response.get("status") == "success":
            raw_data = response.get("data")
            # עטיפה ברשימה כפי שסידרנו קודם
